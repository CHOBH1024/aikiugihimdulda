...
const updates = {
  ...
  '04_wise.md': {
    ...
    deepReflection: {
      question: `나는 왜 통제 불가능 아이의 '감정'을 마주할 때, 자꾸만 '이성적 지식'이라는 방패 뒤로 숨으려 하는가?`,
...
Object.keys(updates).forEach(filename => {
  ...
  // Remove existing extreme sections if any to avoid duplicates
  content = content.replace(/## 💣 팩트 폭격과 뼈 때리는 조언[\s\S]*?(?=## 💊|## 💡|---)/, '');
  content = content.replace(/## 🔬 학술적 심층 분석과 자기 성찰[\s\S]*?(?=## 💊|## 💡|---)/, '');

  const extSection = `
## 💣 팩트 폭격과 뼈 때리는 조언 (Brutal Truth)

> **" ${updates[filename].factBomb} "**

${updates[filename].harshAdvice}
...
### 스스로에게 던지는 돌직구 질문
**Q. ${updates[filename].deepReflection.question}**

### 🧠 심리학적 의미 (Psychological Meaning)
${updates[filename].deepReflection.meaning}

---
`;

  // Inject before "## 💊 내 아이를 위한 맞춤 언어 처방전"
  if (content.includes('## 💊 내 아이를 위한 맞춤 언어 처방전')) {
    content = content.replace('## 💊 내 아이를 위한 맞춤 언어 처방전', extSection + '\n## 💊 내 아이를 위한 맞춤 언어 처방전');
  } else {
    content += extSection;
  }
